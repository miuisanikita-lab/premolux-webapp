...
const PremiumPage = ({ goto }) => {
  ...
  const launch = async () => {
    ...
    if (!orderId) return;   // orderId kelmasa — holat kuzatib bo'lmaydi

    // ── backend'dagi HAQIQIY holatni har 1.5s da so'rab turamiz ──
    // (LaneStatus -> frontend "step" moslashtirilishi)
    const STATUS_STEP = {
    ...
    const poll = async () => {
      ...
      try {
        ...
        const byId = {};
        (data?.lanes || []).forEach(l => { byId[l.id] = l; });

        sLanes(prev => prev.map((l, idx) => {
          // backend Lane'lari order ichida tartib bo'yicha keladi —
          // shu tartibni bizning L massivimizga moslashtiramiz
        ...
        if (allDone && data?.lanes?.length) {
          // yakuniy natijani bir marta qo'llaymiz
          ...
          return;  // polling to'xtaydi
        }
      } catch {
        // vaqtinchalik tarmoq xatosi — keyingi urinishda davom etadi
      }
