...
const request = async (path, { method="GET", body, timeout=12000 } = {}) => {
...
const PremiumPage = ({ goto }) => {
  ...
  const launch = async () => {
    ...
    L.forEach((_,i)=>{
      let t = 0;
      t += rnd(500,1400);
      setTimeout(()=>sLanes(x=>x.map((l,j)=>j===i?{...l,num:fakeNum(),step:1}:l)), t);
      t += rnd(900,1800);
      setTimeout(()=>sLanes(x=>x.map((l,j)=>j===i?{...l,code:String(Math.floor(10000+Math.random()*89999)),step:2}:l)), t);
      t += rnd(700,1400);
      setTimeout(()=>sLanes(x=>x.map((l,j)=>j===i?{...l,step:3}:l)), t);
      t += rnd(900,1900);
      setTimeout(()=>sLanes(x=>x.map((l,j)=>j===i?{...l,step:4}:l)), t);
      t += rnd(800,1600);
      const ok = Math.random() > 0.14;
      setTimeout(()=>{
        sLanes(x=>x.map((l,j)=>j===i?{...l,step:ok?5:4,failed:!ok}:l));
        if (ok) {
          hap.ok(); bump?.();
          const cid = L[i].cardId;
  ...
  const reset = () => { sStep(1); sPid(null); sWant(""); sWarn(""); sPicks({}); sLanes([]); };
...
const LoginFlow = ({ account, onChange }) => {
  ...
  const run=async fn=>{sEr("");sLd(true);await new Promise(r=>setTimeout(r,1200));sLd(false);fn();};
  const send=()=>!phone.match(/^\+\d{9,15}$/)?sEr("Format: +998901234567"):run("/auth/send-code",{phone},()=>sStep("otp"));
  const verify=()=>code.length<5?sEr("5 xonali kod kiriting"):run("/auth/verify-code",{phone,code},()=>{code==="22222"?(sFa2(true),sStep("pass"),hap.warn()):(sStep("done"),onChange(phone),hap.ok(),toast({kind:"ok",title:"Hisob ulandi",note:phone}));});
...
const BotRow = ({ bot, idx, onChange }) => {
  ...
  const conn=()=>{if(!inp.startsWith("@")){hap.err();sE("@ bilan boshlang");return;}onChange({...bot,username:inp,connected:true,active:0});hap.ok();toast({kind:"ok",title:"Bot ulandi",note:inp});sI("");sO(false);sE("");};
  const disc=()=>onChange({id:bot.id,username:"",connected:false,active:0,maxLogins:15,online:false});
  return(
...
const CardsPage=()=>{
  ...
  const addP=()=>{if(!nn.trim()){sNE("Ism kiriting");return;}if(P.find(p=>p.name.toLowerCase()===nn.toLowerCase())){sNE("Bu nom bor");return;}sP(p=>[...p,{id:Date.now(),name:nn.trim(),cards:[]}]);hap.ok();toast({kind:"ok",title:"Shaxs qo'shildi",note:nn.trim()});sNN("");sA(false);sNE("");};
  const addB=n=>{const pid=c.pid;sM(null);setTimeout(()=>push({v:"c",pid,bn:n}),80);};
  const addC=card=>{sP(p=>p.map(x=>x.id===c.pid?{...x,cards:[...x.cards,card]}:x));sFresh(card.id);setTimeout(()=>sFresh(null),1500);hap.ok();api.post(`/people/${c.pid}/cards`,card).catch(e=>toast({kind:'err',title:"Serverga saqlanmadi",note:e.message}));toast({kind:'ok',title:'Karta qo\'shildi',note:`•••• ${card.num.replace(/\s/g,'').slice(-4)}`});};
...
export default function App() {
  ...
  const [people, setPeople] = useState([
    { id:1, name:"Islomov Mansur", cards:[
      { id:1, bankId:"kapital", name:"ISLOMOV MANSUR", num:"4111 1111 1111 1111", exp:"12/27", cvv:"123", used:1, limit:3 },
      { id:2, bankId:"humo",    name:"ISLOMOV MANSUR", num:"9860 1234 5678 9012", exp:"06/26", cvv:"456", used:3, limit:3 },
      { id:4, bankId:"kapital", name:"ISLOMOV MANSUR", num:"4111 2222 3333 4444", exp:"09/28", cvv:"321", used:0, limit:3 },
      { id:5, bankId:"uzcard",  name:"ISLOMOV MANSUR", num:"8600 1111 2222 3333", exp:"09/26", cvv:"654", used:2, limit:3 },
    ]},
    { id:2, name:"Karimov Alisher", cards:[
      { id:3, bankId:"uzcard", name:"KARIMOV ALISHER", num:"8600 9876 5432 1098", exp:"03/28", cvv:"789", used:0, limit:3 },
    ]},
  ]);
  const [role, setRole] = useState("owner");
  const [codes, setCodes] = useState([]);
  const [hist, setHist] = useState(seedHist);   // null | owner | partner | worker

  const [partners, setPartners] = useState([
    { id:"p1", name:"MobiShop",   tgId:"784512309", share:15, balance:1240000, orders:412, goal:2000000, today:12, week:[8,11,9,14,10,13,12] },
    { id:"p2", name:"TezPremium", tgId:"512908447", share:12, balance:640000,  orders:238, goal:2000000, today:7,  week:[5,6,4,9,7,8,7] },
  ]);
  const [workers, setWorkers] = useState([
    { id:"w1", name:"Aziz Rahimov",   tag:"@aziz_op",   parent:"owner", online:true,  today:34, ok:96, last:"hozir",  week:[18,24,21,29,26,31,34] },
    { id:"w2", name:"Bekzod Toshev",  tag:"@bekzod_op", parent:"owner", online:true,  today:27, ok:91, last:"2 daq",  week:[22,19,25,20,24,28,27] },
    { id:"w3", name:"Jasur Karimov",  tag:"@jasur_op",  parent:"p1",    online:false, today:11, ok:84, last:"1 soat", week:[15,17,12,19,14,16,11] },
    { id:"w4", name:"Sanjar Yusupov", tag:"@sanjar_op", parent:"p2",    online:false, today:0,  ok:78, last:"kecha",  week:[9,12,8,14,10,7,0] },
  ]);
  ...
  // ── saqlangan holatni yuklash + serverga "salom" (mock) ──
  useEffect(()=>{
    (async()=>{
      try {
        // Telegram initData ni serverga tekshirtirish (hozircha mock)
        await api.post("/auth/verify", {}).catch(e=>{
          // tarmoq xatosi ilovani to'xtatmasin — lokal holat bilan davom etadi
          if (e.code === "NETWORK") setBootErr(e);
        });
        const r = await window.storage.get("premolux_v1");
  ...
  useEffect(()=>{
    ...
    const t = setTimeout(()=>{
      try {
        window.storage.set("premolux_v1", JSON.stringify({
